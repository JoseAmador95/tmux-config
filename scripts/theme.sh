...
for n in $NAMES; do
  # Palette variables are intentionally generated from the authoritative ordered name list.
...
LIGHT_INK='#ffffff'
DARK_INK='#000000'
# C_* variables are generated by the palette loop above.
# shellcheck disable=SC2154
...
for n in rosewater flamingo pink mauve maroon peach yellow green teal sky sapphire lavender; do
  # The loop name selects one of the generated palette variables.
  # shellcheck disable=SC2154
  eval "c=\$C_${n}"
  # shellcheck disable=SC2154
...
# Keep the actual colour policy in session-color.sh. status-right reads these options directly in
# each client's target-session context, so there is no derived global strip to rebuild.
