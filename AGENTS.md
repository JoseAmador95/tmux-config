...
- The status bar uses the `vscode-modern` palette: the bar itself is `bg=terminal` (no background of its own, so it follows the terminal light↔dark) and the accent `#005FB8` lives only in the pills. Each session's name pill is tinted by `session-color.sh` (stable per name, so `ssh_<host>` is stable per host), published as the session-scoped `@pill` user option. `scripts/fzf-style.sh` carries the same palette for the popups — change both together.
